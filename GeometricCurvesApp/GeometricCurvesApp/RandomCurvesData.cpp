...
const double& MinMaxPositiveDouble::getMin() const
{
	return min;
}

const double& MinMaxPositiveDouble::getMax() const
{
	return max;
}
...
const int& MinMaxPositiveInt::getMin() const
{
	return min;
}

const int& MinMaxPositiveInt::getMax() const
{
	return max;
}
								   ...
								   const MinMaxPositiveDouble& helixSlope)
{
	...
	int resultCirclesCount {randomInIntervalPositiveInt(circlesCount)};
	int resultEllipsesCount {randomInIntervalPositiveInt(ellipsesCount)};
	int resultHelixesCount {randomInIntervalPositiveInt(helixesCount)};
	...
	{
		CurvesTypes currCurveType = static_cast<CurvesTypes>(randomInIntervalPositiveInt(MinMaxPositiveInt(0, 2)));
...
{
	double resultRadius = randomInIntervalDouble(radius);
	circlesPtrs.push_back(std::make_shared<Сircle>(resultRadius));
...
{
	double resultMajorSemiaxis = randomInIntervalDouble(majorSemiaxis);
	double resultSemiminorAxis = randomInIntervalDouble(semiminorAxis);
...
{
	double resultRadius = randomInIntervalDouble(radius);
	double resultSlope = randomInIntervalDouble(slope);
...
void RandomCurvesData::circlePtrsSort()
{
	std::sort(circlesPtrs.begin(), circlesPtrs.end(), 
		[](const std::shared_ptr<Сircle>& right, const std::shared_ptr<Сircle>& left)
